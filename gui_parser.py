# gui_parser_with_dbscan_gui.py
# ---------------------------------------------------------------
# 功能：
#   1️⃣ 实时接收点云帧并保存为 xlsx
#   2️⃣ 自动调用 DBSCAN_generator 聚类
#   3️⃣ 通过 pointcloud_notifier 通知 GUI 绘制聚类结果
# ---------------------------------------------------------------
    ...
    def readAndParseUartDoubleCOMPort(self):
        ...
        if self.saveBinary == 1:
            ...
            if len(self.pointCloudCache) == self.framesPerFile:
                ...
                notify_new_pointcloud(file_xlsx)  # 🔔 通知原始点云显示
                ...
                if self.cluster_dir:
                    try:
                        ...
                        payload = {
                            "type": "clustered",
                            "path": cluster_path,
                            "clusters": [c.tolist() for c in valid_points_list],
                            "noise": noise.tolist() if len(noise) > 0 else []
                        }
                        notify_new_pointcloud(payload)

                        print(f"✅ Cluster saved and sent to GUI: {cluster_path}")
