...
from library.utils import np_filter, np_2D_row_removal, cubehull, boundary_calculator, np_get_idx_bool
...
class BGNoiseFilter:
    def __init__(self, **kwargs_CFG):
        ...
        self.BGN_accept_SNR_threshold = BGN_CFG['BGN_accept_SNR_threshold']
        self.BGN_filter_SNR_threshold = BGN_CFG['BGN_filter_SNR_threshold']
    ...
    def BGN_filter(self, data_points):
        for c in self.BGN_cluster_boundary:
            ...
            noise, _ = np_filter(data_points, idx=0, range_lim=xboundary, mode=0)
            noise, _ = np_filter(noise, idx=1, range_lim=yboundary, mode=0)
            noise, _ = np_filter(noise, idx=2, range_lim=zboundary, mode=0)
            # identify the noise with no speed and low SNR
            noise, _ = np_filter(noise, idx=3, range_lim=0)
            noise, _ = np_filter(noise, idx=4, range_lim=self.BGN_filter_SNR_threshold)
            # remove noise points specifically
            data_points = np_2D_row_removal(data_points, noise)
        return data_points
    ...
    def BGN_get_filter_area(self):
        ...
        for bb in self.BGN_cluster_boundary:
            BGN_block_list.append(cubehull(None, (bb[0], bb[1]), (bb[2], bb[3]), (bb[4], bb[5])))
        return BGN_block_list

    # background noise area update
    def BGN_update(self, bg_noise):
        # identify the noise with no speed and low SNR
        bg_noise, _ = np_filter(bg_noise, idx=3, range_lim=0)
        bg_noise, _ = np_filter(bg_noise, idx=4, range_lim=self.BGN_accept_SNR_threshold)
        ...
        if len(self.BGN_deque) == self.BGN_deque.maxlen and len(BGN_points) > 1000:
            ...
            for u in unique:
                ...
                xyz = boundary_calculator(cluster, idx=range(3))  # get boundary info of cluster
    ...
    def _check_area_overlap(self, database, data_point):
        ...
        idx0, _ = np_get_idx_bool(database, idx=0, range_lim=(None, data_point[0]), mode=0)  # xmin
        idx1, _ = np_get_idx_bool(database, idx=1, range_lim=(data_point[1], None), mode=0)  # xmax
        idx2, _ = np_get_idx_bool(database, idx=2, range_lim=(None, data_point[2]), mode=0)  # ymin
        idx3, _ = np_get_idx_bool(database, idx=3, range_lim=(data_point[3], None), mode=0)  # ymax
        idx4, _ = np_get_idx_bool(database, idx=4, range_lim=(None, data_point[4]), mode=0)  # zmin
        idx5, _ = np_get_idx_bool(database, idx=5, range_lim=(data_point[5], None), mode=0)  # zmax
        idx_inside = (idx0 & idx1 & idx2 & idx3 & idx4 & idx5).any()  # if any points found

        # check if data_point contains any of database
        idx0, _ = np_get_idx_bool(database, idx=0, range_lim=(data_point[0], None), mode=0)  # xmin
        idx1, _ = np_get_idx_bool(database, idx=1, range_lim=(None, data_point[1]), mode=0)  # xmax
        idx2, _ = np_get_idx_bool(database, idx=2, range_lim=(data_point[2], None), mode=0)  # ymin
        idx3, _ = np_get_idx_bool(database, idx=3, range_lim=(None, data_point[3]), mode=0)  # ymax
        idx4, _ = np_get_idx_bool(database, idx=4, range_lim=(data_point[4], None), mode=0)  # zmin
        idx5, _ = np_get_idx_bool(database, idx=5, range_lim=(None, data_point[5]), mode=0)  # zmax
